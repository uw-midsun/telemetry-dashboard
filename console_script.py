...
Includes functions for solar array data to be tested June 9, 2019 at track.
-------------------------------------------------------
Author:  Rebecca Cho
...
def read_drive_output(data): 
	global throttle, direction, cruise_control, mechanical_brake_state
...
def read_battery_vt(data):
	...
	# note that messages may not always start at module 0
...
# 1450
def read_solar_data_front(data):
...
def print_battery_data():	#TODO: print time stamp
	...
	for i in BATTERY_LAYOUT:
		if (module_counter + 1) % 6 == 0:	# print in next line
...
def print_driver_controls_data():
	print('Direction: {:9}  Throttle: {:9}  Brake State: {:9}  Cruise Control State: {:9}  Cruise Control Target: {:9}'\
			.format(DRIVE_OUTPUT_DIRECTION.get(direction), throttle, mechanical_brake_state, cruise_control, cruise_control_target))
